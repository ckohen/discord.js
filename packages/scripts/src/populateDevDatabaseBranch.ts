...
for await (const file of globber.globGenerator()) {
	...
	if (parsed?.groups) {
		...
		try {
			...
			const { url } = await put(`${name.replace('@ckohen/', '')}/${parsed.groups.semver}.json`, data, {
				access: 'public',
				addRandomSuffix: false,
			});
			await pool.sql`insert into documentation (name, version, url) values (${name.replace('@ckohen/', '')}, ${
		...
		try {
			const { name } = JSON.parse(data);
			const { url } = await put(`${name.replace('@ckohen/', '')}/main.json`, data, {
				access: 'public',
				addRandomSuffix: false,
			});
